    ...
    def check_db(self):
        if not self.session and not self.job:
            try:
                ...
                self.session = None
                logging.warning(
                    "Error creating database session: %s" % str(err))
            else:
                try:
        ...
        for patt in params.files_to_save:
            ...
            for file in files:
                ...
                if file_name == "wrfrst_d01_" + datetime2datewrf(params.chunk_rdate):
                    ...
                    logging.info(
                        "Skipping initial restart file %s" % file_name)
                    continue
                    ...
                    if "WRF4G_postprocessor" in output:
                        logging.info(
                            "'%s' was already postprocessed" % file_name)
                        continue
                    try:
                        mo = re.findall(
                ...
                if patt != "wrfrst" and params.wrfout_name_end_date == 'yes' and end_date_file:
                    dest_file = WRFFile(
                        file_name, end_date_file).file_name_out_iso()
                else:
                ...
                try:
                    ...
                    try:
                        ...
                        raise JobError("'%s' has not copied" %
                                       file, Job.CodeError.COPY_OUTPUT_FILE)
                try:
                    os.remove(file)
                except:
                    pass
            ...
            if params.ungribprocessor:
                for pp in params.ungribprocessor.replace(' ', '').split(','):
                    if pp != '':
                        ...
                        if not which("ungribprocessor.%s" % pp):
                            raise JobError("UngribProcessor '%s' does not exist" %
                                           pp, Job.CodeError.UNGRIB_PROCESSOR_FAILED)
                        preprocessor_log = join(
                            params.log_path, 'ungribprocessor.%s.log' % pp)
                        code, output = exec_cmd(
                            "ungribprocessor.%s >& %s" % (pp, preprocessor_log))
